	...
	virtual void run() {
		...
		//for (k = 0; k < 2; k++) {
			...
			ON_FIRST_INVOCATION_BEGIN();
					...
					task1.pin("MUXEN").setEdgeSelect(TMU::RISE).setNumSamples(
							mSamples).setNumMeasurements(1).setNumShots(1);
					task1.pin("MUXEN").setDatarate(1.0 / (datarate * 2));
...
REGISTER_TESTMETHOD("TOFF_ENZ_TMU_MEASURE", TOFF_ENZ_TMU_MEASURE)
;
