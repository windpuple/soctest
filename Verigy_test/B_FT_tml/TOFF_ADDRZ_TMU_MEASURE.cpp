...
class TOFF_ADDRZ_TMU_MEASURE: public testmethod::TestMethod {
protected:
	double mtreshold_a;
	int mPrescaler;
	int mSamples;
...
double						datarate = Primary.getTimingSpec().getSpecValue("per_3250") * 1e-9;
						...
						task1.pin("SENADDR0").setEdgeSelect(TMU::RISE).setNumSamples(mSamples).setNumMeasurements(1).setNumShots(1);
						task1.pin("SENADDR0").setDatarate(1.0/(datarate*2));
						task1.pin("SENADDR0").setAThreshold(mtreshold_a);

						task1.pin("MUX0OUTB").setEdgeSelect(TMU::FALL).setNumSamples(mSamples).setNumMeasurements(1).setNumShots(1);
						...
						for (int j = 0; j < 10; j++) {
							...
							cout << " TOFF ADDRZ Code "<<j<<" MUX0OUT value (via measured value) = " << meanvalue0[j]* 1e9 << " NS" << endl;

							TEST(TMUGROUP[0], "TOFF ADDRZ MUX0 Test", LIMIT(TM::GE, 0.0, TM::LE, 10.0), meanvalue0[j]* 1e9 , TM::CONTINUE);
...
REGISTER_TESTMETHOD("TOFF_ADDRZ_TMU_MEASURE", TOFF_ADDRZ_TMU_MEASURE)
;
