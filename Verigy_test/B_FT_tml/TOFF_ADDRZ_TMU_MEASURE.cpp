...
protected:
	...
	virtual void run() {
		...
		ON_FIRST_INVOCATION_BEGIN();
				...
				task1.pin("SENADDR0").setEdgeSelect(TMU::RISE).setNumSamples(
		...
		ON_FIRST_INVOCATION_BEGIN();
				...
				task2.pin("SENADDR0").setEdgeSelect(TMU::RISE).setNumSamples(
						mSamples).setNumMeasurements(1).setNumShots(1);
		...
		ON_FIRST_INVOCATION_BEGIN();
				...
				task3.pin("SENADDR0").setEdgeSelect(TMU::RISE).setNumSamples(
						mSamples).setNumMeasurements(1).setNumShots(1);
		...
		for (int j = 0; j < 98; j++) {
			...
			cout << " TOFF ADDRZ Code " << j
					<< " MUX0OUT value (via measured value) = "
					<< meavalue0_final[j] * 1e9 << " NS" << endl;

			TEST(TMUGROUP[0], "TOFF ADDRZ MUX0 Test", LIMIT(TM::GE, 0.0,
					TM::LE, 10.0), meavalue0_final[j] * 1e9, TM::CONTINUE);
